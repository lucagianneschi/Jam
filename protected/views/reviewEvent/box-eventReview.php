...
$reviews = ReviewEvent::model()->eventPage($id, $limit, $skip);
...
if ($reviews || isset($_SESSION['id'])) {
    ...
    <div class="row" id="social-EventReview">
		...
		foreach ($reviews as $key => $value) {
		    ...
		    $review_data = ucwords(strftime("%A %d %B %Y - %H:%M", $value['createdat']->getTimestamp()));
	    	...
	    	<div class="row" id="social-EventReview-<?php echo $review_objectId; ?>">
	    		...
	    		<div class="box">
	    			...
	    			<div class="row " style=" margin-top:10px;">						
	    				...
	    				<a href="#" class="orange no-display viewText"><strong onclick="toggleText(this, 'eventReview_<?php echo $i ?>', '<?php echo $review_text ?>');"><?php echo Yii::t('string', 'view.viewall'); ?></strong></a>
	    				<a href="#" class="orange no-display closeText"><strong onclick="toggleText(this, 'eventReview_<?php echo $i ?>', '<?php echo $review_text ?>');"><?php echo Yii::t('string', 'view.close'); ?></strong></a>
	    			    </div>
	    			</div>
	    			<div class="row">
	    			    <div  class="large-12 columns">
	    			...
	    			<div class="row eventReview-propriety">
	    				...
	    				<div class="small-6 columns ">
	    				    ...
	    				    <a class="note grey" onclick="loadBoxOpinion('<?php echo $review_objectId; ?>', '<?php echo $review_user_objectId; ?>', 'Comment', '#social-EventReview-<?php echo $review_objectId; ?> .box-opinion', 10, 0)"><?php echo Yii::t('string', 'view.comment'); ?></a>
	    				    <!-- a class="note grey" onclick="setCounter(this,'<?php echo $review_objectId; ?>','EventReview')"><?php echo Yii::t('string', 'view.share'); ?></a -->
	    				</div>
	    				<div class="small-6 columns propriety ">
		...
		<div class="row otherSet">
			...
			?>
			<div class="text" onClick="loadBoxEventReview(<?php echo $limit + $reviewToShow; ?>, 0);"><?php echo Yii::t('string', 'view.other_rew'); ?></div>
		    </div>
		</div>
		<?php
	    }
		...
		<div class="row">
			...
			<div class="box">
			    <div class="row">
				<div class="large-12 columns"><p class="grey"><?php echo Yii::t('string', 'view.media.event.eventreview.nodata'); ?></p></div>
			    </div>
			</div>
		    </div>
		</div>
